...
// Softbankの定義がないので追加
static constexpr char SOFTBANK_LTEM_BAND[] = "0x81";  // LTE B1 | LTE B8
...
static constexpr int OPERATOR_ACT = 7;  // eMTC

static constexpr const char* APN = "iot.1nce.net";
...
static constexpr int COPS_AUTO_SELECT_TIMEOUT_MS = 1000 * 30;
static constexpr int COPS_MANUAL_SELECT_TIMEOUT_MS = 1000 * 30;
...
static std::string GetConfiguredOperatorPlmn() {
  if (OPERATOR_PLMN == nullptr) {
    return "";
  }
  return Trim(OPERATOR_PLMN);
}

static bool HasOperatorPlmnSelection() {
  return !GetConfiguredOperatorPlmn().empty();
}

static bool IsValidPlmn(const std::string& plmn) {
  if (plmn.empty()) {
    return true;
  }
  ...
  return IsDigitsOnly(plmn);
}

static bool IsAtCommandTimeoutResult(WioCellularResult result) {
  return result == WioCellularResult::WaitCommandTimeout || result == WioCellularResult::ReadResponseTimeout;
}

static WioCellularResult ConfigureOperatorSelection() {
  Serial.println("  - AT+COPS=3,2");
  const auto formatResult = WioCellular.executeCommand("AT+COPS=3,2", COPS_TIMEOUT_MS);
  if (formatResult != WioCellularResult::Ok) {
    return formatResult;
  }

  const auto plmn = GetConfiguredOperatorPlmn();
  if (plmn.empty()) {
    Serial.printf("  - AT+COPS=0 (timeout=%dms)\n", COPS_AUTO_SELECT_TIMEOUT_MS);
  ...
  Serial.printf("  - %s (timeout=%dms)\n", command.c_str(), COPS_MANUAL_SELECT_TIMEOUT_MS);
...
static std::string BuildOneNcePdpAuthCommand() {
  // 1NCE: 認証なし (必要時は ",1,\"\",\"\"" のPAPへ変更)
  return "AT+CGAUTH=" + std::to_string(PDP_CONTEXT_ID) + ",0";
}
...
void setup() {
  ...
  Serial.println();
  Serial.println("Startup 1NCE attach check");
  ...
  Serial.printf("Search LTE-M band: %s\n", LTEM_BAND);
  const auto operatorPlmn = GetConfiguredOperatorPlmn();
  if (operatorPlmn.empty()) {
    Serial.println("Operator selection: auto");
  } else {
    Serial.printf("Operator selection: manual (PLMN=%s act=eMTC)\n", operatorPlmn.c_str());
  }
  Serial.printf("Carrier profile: 1nce (APN=%s authType=0)\n", APN);
}
...
void loop() {
  ...
  if (!initialized) {
    ...
    const auto cgauthResult = WioCellular.executeCommand(BuildOneNcePdpAuthCommand(), CGAUTH_TIMEOUT_MS);
    if (cgauthResult != WioCellularResult::Ok) {
      Serial.printf("ERROR: AT+CGAUTH failed: %s\n", WioCellularResultToString(cgauthResult));
      abort();
    }

    Serial.println("Step: configure operator selection");
    const auto operatorResult = ConfigureOperatorSelection();
    if (operatorResult != WioCellularResult::Ok) {
      if (HasOperatorPlmnSelection() && !IsAtCommandTimeoutResult(operatorResult)) {
        Serial.printf("ERROR: operator selection failed: %s\n", WioCellularResultToString(operatorResult));
        abort();
      }
      Serial.printf("WARN: operator selection failed: %s (continue)\n", WioCellularResultToString(operatorResult));
    }
